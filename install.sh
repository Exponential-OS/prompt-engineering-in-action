...
REPO="${CO_DIALECTIC_REPO:-https://raw.githubusercontent.com/Exponential-OS/prompt-engineering-in-action/main}"
VERSION="4.9.0"
...
fetch_skill_extras() {
    ...
    case "$skill_name" in
        judge-panel)
            ...
            if fetch_repo_file "$REPO/plugins/co-dialectic/skills/judge-panel/scripts/judge_panel.py" "$skill_dir/scripts/judge_panel.py"; then
...
install_fish_gate() {
    ...
    if fetch_repo_file "$REPO/plugins/co-dialectic/fish/HOW.py" \
            "$FISH_INSTALL_DIR/HOW.py"; then
    ...
    if fetch_repo_file "$REPO/plugins/co-dialectic/fish/hooks/claude-code.py" \
            "$FISH_INSTALL_DIR/hooks/claude-code.py"; then
